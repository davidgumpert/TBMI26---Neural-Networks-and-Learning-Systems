...
for n = 1:numIterations
    ...
    [Y, U, L] = runMultiLayer(XTrain, Wout, Vout);
    grad_w = -2*[tanh(XTrain*Vout) ones(size(XTrain, 1), 1)]'*(DTrain - [tanh(XTrain*Vout) ones(size(XTrain, 1), 1)]*Wout); % Gradient for the output layer
    grad_v = -2*XTrain'*(((DTrain - (tanh(XTrain*Vout)*Wout(1:end-1, :)))*Wout(1:end-1, :)').*(1-tanh(XTrain*Vout).^2)); % And the input layer
    
    grad_w = grad_w/size(XTrain,1);
    grad_v = grad_v/size(XTrain,1);
